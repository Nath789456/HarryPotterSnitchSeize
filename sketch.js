...
gameOver= createSprite(width/2,height/2-80,50,50);
...
function draw() { 
 ...
 textStyle(ITALIC);
 textSize(30);
 text("The Harry Potter Snitch Seize Game",100,height/2-200);
 fill("white");
 textSize(24);
 fill("yellow");
 text("Press on the up and down arrow keys to move Harry!!",100,height/2+200);
 textSize(20);
 textFont('Helvetica');
 fill("white");
 text("Distance Covered: " + score,width/2+100,height/2-200);
 text("Snitches Caught: " + snitchScore,width/2+100,height/2-150);
}
...
function spawnDementors() {
  if(frameCount % 100 === 0) {
    var dementor = createSprite(width,height-250,10,40);
    //obstacle.debug = true;
...
function spawnSnitch(){
  if(frameCount %300 ===0){
    var snitch= createSprite(width,height-250,20,30);
